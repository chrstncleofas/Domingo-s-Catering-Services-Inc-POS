...
Public Class SalesReportView
    ...
    Private Sub btnLoad_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnLoad.Click
        ...
        If cmbFilter.Text = "Load All Data" Then
            ...
            For i As Integer = 0 To dgSales.RowCount - 1
                salesTotal += dgSales.Rows(i).Cells(3).Value.ToString
            Next
            lblTotalSales.Text = FormatCurrency(salesTotal)
            ' End codes for computation
        ElseIf cmbFilter.Text = "Filter by Date" Then
            Try
                tbltran = New OleDb.OleDbDataAdapter("SELECT [Trans ID],[Item Name],[Quantity],[Total],[Date Time],[Cashier Name],[Payment Method] FROM tbltran WHERE [Date Time] BETWEEN #" & date1.ToString("MM/dd/yyyy") & " " & time1.ToString("HH:mm:ss tt") & "# and #" & date2.ToString("MM/dd/yyyy") & " " & time2.ToString("HH:mm:ss tt") & " # ORDER BY [Date Time] DESC", db)
                dbds = New DataSet()
                ...
                If dbds.Tables("tbltran").Rows.Count > 0 Then
                    ...
                    'Computation in Sales
                    For i As Integer = 0 To dgSales.RowCount - 1
                        salesTotal += dgSales.Rows(i).Cells(3).Value
                    Next
                    lblTotalSales.Text = FormatCurrency(salesTotal)
                    ' End codes for Computation
                    TabPage2.Show()
    ...
    Private Sub btnVoid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnVoid.Click
        opendb6()
        sqlquery6()
        dgSales.DataSource = dbds.Tables("tblvoid")
        Dim report As New VoidItem
        'Crystal Reporting Codes
        report.SetDataSource(dbds)
        CrystalReportViewer1.ReportSource = report
